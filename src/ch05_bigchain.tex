...
Instead, it has a list of votes, and each vote has a reference to the id/hash of the previous block.

If for some reason, the nodes disagree on the parent (e.g.~malicious nodes) one node can just mark the block as invalid and return the transactions to the backlog to try again.
We say that, ``Chainification happens at \textit{voting} time.''
